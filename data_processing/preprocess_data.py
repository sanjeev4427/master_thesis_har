...
def load_dataset(dataset, pred_type='actions', include_null=False, saving_type='min'):
    """
    ...
    if dataset == 'wetlab':
        if pred_type == 'actions':
            class_names = ['cutting', 'inverting', 'peeling', 'pestling', 'pipetting', 'pouring', 'pour catalysator', 'stirring',
                           'transfer']
        ...
        sampling_rate = 50
        has_null = True
    elif dataset == 'trimm_dich':
        class_names = ["rope", "leg_raises", "dips", "pull_ups", "hyper_extensions", "crunches",
                       "stretching", "push_ups", "hanging"]
        sampling_rate = 50
        has_null = True
    elif dataset == 'sbhar':
...
def adjust_labels(data_y, dataset, pred_type='actions'):
    ...
    if dataset == 'wetlab':
        ...
        data_y[data_y == 'rope'] = 1
        data_y[data_y == 'leg_raises'] = 2
        data_y[data_y == 'dips'] = 3
        data_y[data_y == 'pull_ups'] = 4
        data_y[data_y == 'hyper_extensions'] = 5
        data_y[data_y == 'crunches'] = 6
        data_y[data_y == 'stretching (pole)'] = 7
        data_y[data_y == 'stretching (legs)'] = 7
        data_y[data_y == 'push_ups (dips)'] = 8
        data_y[data_y == 'push_ups (ground)'] = 8
        data_y[data_y == 'hanging (pull_up)'] = 9
        data_y[data_y == 'hanging (bar)'] = 9
        data_y[data_y == 'drinking'] = 0
    elif dataset == 'sbhar':
