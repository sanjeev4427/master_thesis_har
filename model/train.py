...
def train(train_features, train_labels, val_features, val_labels, network, optimizer, loss, config, log_date, log_timestamp, lr_scheduler=None):
    ...
    for e in range(config['epochs']):
        ...
        with torch.no_grad():
            ...
            mod_val_preds = np.copy(val_preds)
            ...
            print("EPOCH: {}/{}".format(e + 1, config['epochs']),
                  ...
                  "Data saved (R): {:.2%}\n".format(sum(data_saved) / (sum(count_array) * (1 - config['sw_overlap'] * 0.01) * config['sw_length'])),
                  "Mod Valid Acc (M): {:.4f}".format(jaccard_score(val_gt, mod_val_preds, average='macro')),
                  "Mod Valid Prc (M): {:.4f}".format(precision_score(val_gt, mod_val_preds, average='macro')),
                  "Mod Valid Rcl (M): {:.4f}".format(recall_score(val_gt, mod_val_preds, average='macro')),
                  "Mod Valid F1 (M): {:.4f}".format(f1_score(val_gt, mod_val_preds, average='macro')),
                  )
    ...
    if config['valid_epoch'] == 'best':
        return best_network, checkpoint, np.vstack((best_val_preds, val_gt)).T, np.vstack((best_val_preds, val_gt)).T,\
               np.vstack((best_train_preds, train_gt)).T, computations_saved, data_saved, count_array, \
               count_array * (1 - config['sw_overlap'] * 0.01) * config['sw_length']
    else:
        checkpoint = {
            "model_state_dict": network.state_dict(),
        ...
        return network, checkpoint, np.vstack((val_preds, val_gt)).T, np.vstack((mod_val_preds, val_gt)).T, \
               np.vstack((train_preds, train_gt)).T, computations_saved, data_saved, count_array, \
               count_array * (1 - config['sw_overlap'] * 0.01) * config['sw_length']

#! where is test dataset 
